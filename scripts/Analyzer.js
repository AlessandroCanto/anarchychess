CHESSAPP.Analyzer ={
	...
	makeAllOptions : function(settings){
		...
		//extends the STG object with settings properties
		...
		var resp = {
			...
			kingInCheck : false,
			//store the moves of the current player
			allOptions : []
		};
		var r,
		...
		for(var i = 0; i < pieces.length; i++){
			//if piece is real and is of type king
			if(pieces[i] && pieces[i].pieceType == "king"){
				//assigns an index and determines whether it is white/black
				if(pieces[i].color == "W"){
					whiteKingIndex = i;
				}
				else{
	...
	getOptions : function(settings){
		...
		switch(type){
			...
			if(stg.castleTest){
					...
					if ((curx + 1 != 2 || cury != 1) && (curx + 1 != 2 || cury != 6)) mk(curx + 1, cury, true, true);
					if ((curx + 1 != 2 || cury - 1 != 1) && (curx + 1 != 2 || cury - 1 != 6)) mk(curx + 1, cury - 1, true, true);
					if ((curx != 2 || cury + 1 != 1) && (curx != 2 || cury + 1 != 6)) mk(curx, cury + 1, true, true);
					if ((curx != 2 || cury - 1 != 1) && (curx != 2 || cury - 1 != 6)) mk(curx, cury - 1, true, true);
				}
			...
			case "bishop":
				...
				break;
			case "rook":
				if (type == "rook" && piece.numOfMoves == 0) {
					// Check if the pawns on both sides of the board in the same row as the rook haven't moved
					var sidePawns = [
						CHESSAPP.Analyzer.pieceExists({ pieces: pieces, x: 0, y: (color == 'W') ? 1 : 6 }),
						CHESSAPP.Analyzer.pieceExists({ pieces: pieces, x: 7, y: (color == 'W') ? 1 : 6 })
					];
				
					var pawnsUnmoved = sidePawns.every(pawn => pawn && pawn.pieceType == "pawn" && pawn.numOfMoves == 0);
				
					if (pawnsUnmoved) {
						// Check for enemy rooks on the same file and capture them
						for (var i = 0; i <= 7; i++) {
							if (i != cury) {
								var targetPiece = CHESSAPP.Analyzer.pieceExists({ pieces: pieces, x: curx, y: i });
								if (targetPiece && targetPiece.pieceType == "rook" && targetPiece.color != color) {
                    mk(curx, i, false, true);
                }
            }
